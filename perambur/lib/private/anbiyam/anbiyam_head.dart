...
class AnbiyamHeadScreen extends StatefulWidget {
  const AnbiyamHeadScreen({Key? key}) : super(key: key);

  @override
  State<AnbiyamHeadScreen> createState() => _AnbiyamHeadScreenState();
}

class _AnbiyamHeadScreenState extends State<AnbiyamHeadScreen> {
  ...
  getAnbiyamHeadsData() async {
    var request = http.Request('GET', Uri.parse("$baseUrl/res.parish.bcc"));
    request.body = json.encode({
      "params": {
        "filter": "[['parish_id','=',${int.parse(parishID)}],['id','=',$bccId]]",
        "access_all": "1",
                                ...
                                Padding(
                                  ...
                                  child: GestureDetector(
                                    onTap: () {
                                      ...
                                      } else if (int.parse(familyId) == members[index]['member_id']['family_id']['id']) {
                                        indexValue = members[index]['member_id']['id'];
                                        indexName = members[index]['member_id']['member_name'];
                                        assignMemberDetails(indexValue, indexName);
                                      } else {
                                ...
                                Positioned(
                                  ...
                                  child: GestureDetector(
                                    onTap: () {
                                      ...
                                      } else if (userRole == 'Parish Family' && int.parse(familyId.toString()) == members[index]['member_id']['family_id']['id']) {
                                        indexValue = members[index]['member_id']['family_id']['id'];
                                        indexName = members[index]['member_id']['family_id']['name'];
                                        assignValues(indexValue, indexName);
                                      } else {
