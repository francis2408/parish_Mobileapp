...
class _AllEventScreenState extends State<AllEventScreen> {
  ...
  getEventData() async {
    var request = http.Request('GET', Uri.parse('$baseUrl/calendar.event'));
    ...
    request.body = json.encode({
      "params": {
        "filter": selectedTab == "This Week" ? "[['parish_id','=',$parishID],['category','=','Calendar'],['start','>=','$today'],['stop','<=','$weekEndDate']]" : selectedTab == "This Month" ? "[['parish_id','=',$parishID],['category','=','Calendar'],['start','>=','$monthStartDate'],['stop','<=','$monthEndDate']]" : "[['parish_id','=',$parishID],['category','=','Calendar']]",
        "query": "{id,name,start,stop,duration,start_date,stop_date,allday,type,location,description,session}",
        "order": selectedTab == "All" ? "start desc" : selectedTab == "This Month" ? "start desc" : "start asc"
      }
    });
